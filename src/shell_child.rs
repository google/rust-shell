...
///    via `ShellHandle`.
///  - `RwLock` to `signal()` callable while `wait_null()` is blocking(). Both
///    `signal()` and `wait_null()` reguires the read lock
///    which can be obtained by multiple threads at the same time.
